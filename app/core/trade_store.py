...
class TradeType(StrEnum):
    """Trade classification for metric isolation.

    STRATEGY: Trades generated by the trading engine (live_loop, shadow_loop).
    RECONCILED: Trades created by position reconciler to match exchange state.
    BACKFILL: Trades created by backfill to fill historical gaps.
    """
...
class TradeStore:
    ...
    async def record_entry(
        ...
        trade_type: TradeType = TradeType.STRATEGY,
    ) -> int:
            ...
            async with self.db.engine.connect() as conn:
                result = await conn.execute(
                    text(
                        ...
                        cvd_slope, spread_bps, session_mult, regime_encoded, atr_pct, vol_factor, trade_type)
    ...
    async def record_full_trade(
        ...
        exit_time: datetime,
        exit_reason: str,
        trade_type: TradeType = TradeType.BACKFILL,
    ) -> int:
            ...
            async with self.db.engine.connect() as conn:
                result = await conn.execute(
                    text(
                        ...
                        pnl, regime, entry_time, exit_time, exit_reason, trade_type)
