"""Checkpoint Manager — evaluate time-based exits and hard stops.
    ...
    async def _evaluate(self, pos: dict, price_getter, state_manager) -> None:
        ...
        if is_checkpoint_review and min_pnl is not None:
            ...
            if decay_factor < Decimal("1.0") and atr > 0:
                logger.info(
                    f"Position Confidence Decay: {symbol} reached {new_checkpoint}, "
                    f"tightening SL (factor: {decay_factor})"
                )
            ...
            if pnl_pct < min_pnl:
                logger.warning(
                    f"Checkpoint {new_checkpoint}: {symbol} {side} underperforming "
                    f"(pnl={pnl_pct:.4f} < min={min_pnl:.4f})"
                )
                ...
                if self.position_judge:
                    ...
                    if verdict:
                        logger.info(
                            f"PositionJudge (Checkpoint): {symbol} {side} -> "
                            f"{verdict.action} ({verdict.tier_used})"
                        )
                        if self.trade_store:
                            try:
                                import json
    ...
    async def _exit(
        ...
        try:
            ...
            if self.trade_store:
                if exit_price <= 0:
                    logger.error(
                        f"REFUSING to close trade with exit_price=0 for {symbol} {side}. "
                        f"Order may have failed. Order ID: {order_id}"
                    )
                    if self.alert_service:
                        try:
                            await self.alert_service.send(
                                f"CRITICAL: {symbol} exit_price=0 — manual review needed. "
                                f"Order ID: {order_id}"
                            )
                        except Exception:
                            pass
            ...
            if self.trade_memory:
                try:
                    ...
                    if entered_str:
                        entered_at = datetime.fromisoformat(entered_str)
                        hold_min = int(
                            (datetime.now(timezone.utc) - entered_at).total_seconds() / 60
                        )
                    regime = pos.get("regime", "UNKNOWN")
                    entry_conf = (
