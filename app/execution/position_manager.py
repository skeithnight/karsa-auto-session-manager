                ...
                for pos in positions:
                    ...
                    if current_sl <= 0 and entry_price > 0 and initial_risk > 0:
                        ...
                        if side == "LONG":
                            ...
                            sl_price = entry_price + initial_risk
                        try:
                            await self._client.set_trading_stop(symbol, api_side, stop_loss=sl_price)  # type: ignore[attr-defined]
                            await self._store.update_fields(symbol, side, {  # type: ignore[attr-defined]
    ...
    async def _reconcile_position(self, pos: dict[str, Any]) -> bool:
                ...
                if exch_sl and str(exch_sl) not in ("0", "None", ""):
                    ...
                    if entry_val > 0:
                        if side == "LONG" and sl_val >= entry_val:
                            self._log.warning(f"APM reconcile: {symbol} SL {sl_val} >= entry {entry_val} for LONG — skipping")
                        elif side == "SHORT" and sl_val <= entry_val:
                            self._log.warning(f"APM reconcile: {symbol} SL {sl_val} <= entry {entry_val} for SHORT — skipping")
                        else:
            ...
            try:
                ...
                if hasattr(self._client, "session") and self._client.session:
                    ...
                    raw = await self._client._execute(
                        ...
                        category="linear", symbol=bybit_symbol, interval="60", limit=60,
                    )
        ...
        if changed:
            try:
                ...
                changed_fields = [
                    f for f in ["entry_price", "amount", "current_sl", "stop_loss",
                                "take_profit", "atr", "entry_regime", "regime",
                                "initial_risk_per_unit", "entry_time"]
                    if pos.get(f)
                ]
                await self._store.redis.set(redis_key, _json.dumps(pos))  # type: ignore[attr-defined]
                self._log.warning(
                    "APM reconcile: %s updated %d field(s): %s",
                    symbol, len(changed_fields), changed_fields,
                )
            except Exception:
                ...
                if sl_valid and new_sl > 0:
                    try:
                        ...
                        self._log.warning(
                            ...
                            symbol, side, pos.get("current_sl"), new_sl, float(sl_loss),
                        )
                    except Exception:
