...
class SmartOrderRouter:
    ...
    def __init__(self, bybit_client: BybitClient, max_reprice_attempts: int = 2, reprice_delay_seconds: float = 2.0) -> None:
    ...
    async def execute(
        ...
        sl_distance_pct: Decimal = Decimal("0.02"),
        ...
        if self.skip_to_market:
            ...
            try:
                ...
                await self._place_sl_after_fill(symbol, side, price, amount, sl_distance_pct)
                return market_order
        ...
        for attempt in range(self.max_reprice_attempts):
            ...
            try:
                ...
                if order.get("status") == "open":
                    ...
                    await self._place_sl_after_fill(symbol, side, current_price, amount, sl_distance_pct)
    ...
    async def _place_sl_after_fill(
        ...
        try:
            if side == "buy":
                sl_price = fill_price * (Decimal("1") - sl_distance_pct)
            else:
                sl_price = fill_price * (Decimal("1") + sl_distance_pct)
